...
- レシートOCR → v0.2 で別フェーズ。本命は **写真→OCR→〈金額・店名・購入日時〉→ 店名ベースの散財インサイト**
  （§11 設計メモ参照）。座標/EXIF は使わず端末内完結＝外部送信なし。エンジンは**端末内**（クラウドVisionは採らない）。
...
- 「外部送信は一切しない」を**崩さない**。OCR・名寄せ・集計はすべて**端末内完結**。逆ジオコーディング等の外部API呼び出しは**しない**。
- **OCR エンジン = Apple Vision（端末内）に確定**（2026-06-15 スパイクで実レシート検証済み、§11.7）。クラウドVisionは採らない。
  Web ブラウザ版では動かない（アプリ専用）ため、Capacitor プラグインでネイティブ Vision を JS から呼ぶ（スパイク2）。
  将来 Android も出すなら ML Kit（同・端末内）を同方式で足せる。
...
- ~~OCR エンジン確定~~ → **Apple Vision に確定**（§11.7）。残るは Capacitor プラグイン配線（スパイク2）。
