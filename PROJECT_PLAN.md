...
**進捗（2026-06-28）**: ②TS の能力分岐シーム（`extractionTier`/`extractReceipt`、`OcrProvider.capabilities`）実装・`pnpm verify`/`check:ocr` 緑。
Swift に `getCapabilities`（現状 `onDeviceLLM:false`）＋ `usesLanguageCorrection=false`（要 Xcode 再ビルド）。
**未了**: ①ネイティブのドキュメントスキャナ化（`VNDocumentCameraViewController`）③対応機向け Foundation Models 抽出
④実機（iPhone 11 Pro Max）で撮影→プレフィル精度確認 ⑤`merchantKey` 複数店検証 / 店別インサイト画面（段階B本体）。
